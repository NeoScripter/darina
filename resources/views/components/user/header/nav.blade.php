<div class="flex items-center justify-between gap-5 p-4 sm:px-12 sm:py-5 sm:gap-12">
    ...
    <nav class="items-center hidden md:flex md:gap-10 md:ml-auto">
        <a href=""
            class="block text-sm transition-colors duration-300 shrink-0 hover:text-orange">Отзывы</a>
        <button
        @click="showVideoModal = true; videoModal = 0"
            class="block text-sm transition-colors duration-300 shrink-0 hover:text-orange">Процесс заказа</button>
        <button
            @click="showVideoModal = true; videoModal = 4"
            class="block text-sm transition-colors duration-300 shrink-0 hover:text-orange">Материалы</button>
        <button
            @click="showVideoModal = true; videoModal = 3"
            class="block text-sm transition-colors duration-300 shrink-0 hover:text-orange">О фабрике</button>
        <a href=""
            class="block text-sm transition-colors duration-300 shrink-0 hover:text-orange">Контакты</a>
    </nav>
